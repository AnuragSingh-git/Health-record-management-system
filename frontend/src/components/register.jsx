...
function register() {
  ...
  return (
    ...
    <form className='flex flex-col h-1/2 w-1/2 rounded-xl items-center justify-center border bg-amber-50 gap-4' onSubmit={handlesubmit}>
        <input onChange={(e)=>{setname(e.target.value)}} className='border h-10 w-60 rounded-lg text-center' type='text' placeholder='name'/>
        <input onChange={(e)=>{setemail(e.target.value); console.log(email)}} className='border h-10 w-60 rounded-lg text-center' type='text' placeholder='email'/>
